...
| `review-strategy` | strategy review, review creator strategy, advisory strategy review | drafted strategy, schedule scaffold, broad Research Findings/Evidence, and Creator Profile (never the authoring conversation) | `reviews/<review_record_id>.json` (advisory workspace-level ReviewRecord; never blocks) | yes |
