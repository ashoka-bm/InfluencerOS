...
| `review-quarter-plan` | quarterly review, review draft quarter plan, advisory quarter plan review | complete draft Quarter Plan packet, validating Research Evidence/Findings, Creator Profile, and on repeats the prior Review Record plus unresolved Demand set (never the authoring conversation) | `reviews/<review_record_id>.json` (advisory workspace-level ReviewRecord with capped Demand-loop lineage; never blocks) | yes |
| `quarterly-planning-cycle` | quarterly planning cycle, run the quarter, quarter plan, retrospective and next-quarter concepts | PerformanceSummaries, distilled creator lessons, per-Campaign research, current Campaigns/Concepts, Creator Profile | one approved Quarter Plan via `scaffold quarter-plan`; then applied Campaign lifecycle, Duration Target, and schedule changes plus constructor-built Revisions that point back to the plan; final ready check | yes |
| `review-concept-promotion` | concept review, review weekly candidate packages, advisory candidate review | Creator Profile, one `candidates_ready` Anchor Slot whose `research_state.candidate_content_opportunity_ids` names its exact 2-3-candidate packet even when a Research Run spans slots, matching canonical queue-entry refs, Research Findings/Evidence (never the authoring conversation) | point-in-time `reviews/<review_record_id>.json` via `scaffold review-record` before topic selection (advisory workspace-level concept ReviewRecord; never blocks) | yes |
| `weekly-planning-cycle` | weekly planning cycle, plan coming-week Anchor Slots, batch Concept Approvals | the week's scheduled Campaign Concepts, coming-week Anchor Slots, Creator Profile, scheduled-needs research, candidate Content Opportunities | advisory Concept Review before topic selection/assignment; direct Concept choices refresh canonical research via `refresh-concept-research`; then maintained slot research state plus the batch of human Concept Approvals and Projects through `approve-concept`; final ready check | yes |
