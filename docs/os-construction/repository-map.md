...
| `influencer_os/semantic_lookup.py` | Rebuildable SQLite FTS5 semantic lookup projection (ADR 0011 keyword leg, Decision 1 phasing): `rebuild-lookup` chunks the creator-scoped allowlist (brand context, findings, stable findings, creator learnings, index-allowed performance-summary narratives — never `analytics/`) into heading-aware chunks with line provenance, authority weights, and sha256 change detection; `query-lookup` reranks BM25 candidates by authority and recency behind a hard creator-scope boundary on a read-only connection; never a validation dependency. |
