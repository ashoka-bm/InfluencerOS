...
No provider-backed call may happen without human authorization. An approved
Visual Continuity Plan authorizes one initial generation pass over exactly its
listed creator-setup reference images with no second confirmation. Scope
changes, regeneration, video, audio, voice, render, upload, and out-of-plan
assets require fresh exact call/batch approval.

After plan approval, derive runtime records with
`python3 -m influencer_os derive-setup-reference-approvals <workspace>
--provider <id> --model <model> --cost-note <notice>`, then consume each record
once with `python3 -m influencer_os dispatch-reference-generation <workspace>
<approval-record-id>`. For a pre-ADR-0043 workspace, first run
`python3 -m influencer_os migrate-visual-foundation <workspace>`; the migration
is idempotent and fails rather than guessing an ambiguous avatar binding.
...
- `references/brand/personal-brand-board.json` and `.html`, for every creator
- `progress/setup-checklist.md`
- `context/MEMORY.md`
...
InfluencerOS v1 still prioritizes universal visual-first social posts and universal short-form vertical video. If a creator is text-first or article-first, Creator Setup should record that strategy and avoid unnecessary content-image assets. It must still create the universal profile-avatar asset and tokenized personal brand board used by public profiles and publication design.
...
|---|---|---|
| X, LinkedIn, Substack, Medium, blog/newsletter, Reddit text posts | text | Requires voice, publication style, audience language, topic/pillar strategy, disclosure rules, a profile-avatar asset, and an approved tokenized personal brand board. |
...
- a prompt-staged or available social-profile avatar Reference Asset
- a validated and separately approved tokenized personal brand board
...
For a text-first creator, the minimum shifts to:

- brand voice guide
- author identity notes
- publication design reference
- article or newsletter style reference
- required prompt-staged or available profile avatar reference (person, symbol,
  monogram, product, or brand mark)
- validated and separately approved tokenized personal brand board; production
  spaces may be empty when the creator has no filming or photography world
...
Planned or prompted assets should still use stable asset IDs so downstream plans can name what is missing. The approved Visual Continuity Plan authorizes one initial pass over its listed setup references; other provider-backed generation remains blocked until the user approves the exact call or batch.
...
- reference-asset file existence, foundation completeness, and onboarding readiness milestones: `validate workspace` enforces the readiness blockers at `profile_ready`, `foundation_ready`, `strategy_ready`, `production_ready`, and `active` — selected channels, populated foundation files with required sections and lower-bound word/sample floors, no `TBD` placeholders, always-loaded context byte caps, at least one source intake, a prompt-staged or available profile-avatar asset and approved personal brand board for every creator, required additional asset kinds per declared content medium, lifecycle-appropriate asset/prompt file existence with workspace containment, media generation permission requirements, approved strategy records, conversion-asset provenance and production approval, and content-schedule integrity.
