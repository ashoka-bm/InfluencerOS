...
- `memory/learnings.md` stores distilled creator lessons via `python3 -m influencer_os log-learning <workspace>/memory/learnings.md "<topic>" "<lesson>" --evidence <record-id> ... --strength <strength>` (the `distill-creator-learning` skill's writer). Lessons live under `## Creator Lessons`, grouped by `### <topic>` headings mirroring the PerformanceSummary `applies_to` vocabulary, one parseable entry per lesson: `- YYYY-MM-DD [strength]: lesson (evidence: id, ...)`. Strength is the ADR 0008 scope judgment (`single_post_signal`, `multi_post_pattern`, `weak_signal` — pinned to the PerformanceSummary enum). Every evidence id must resolve to a workspace record (performance summary, published post record, analytics snapshot, project, or output package); the write fails on a dangling id and `validate workspace` re-checks the same rule at rest. The evidence itself stays in the owning project.
