...
- Do not make provider-backed image, video, audio, or render calls without explicit user approval, with two recorded creator-setup carve-outs: setup auto-generates the one bounded Avatar Image call under a system-derived single-use approval record (ADR 0045), and an approved Visual Continuity Plan grants standing approval for one initial generation pass over exactly its listed remaining creator-setup reference assets, with no second confirmation (ADR 0043, carried forward by ADR 0045). Every scope change, regeneration, production-content call, video, voice, render, upload, or asset outside those bounds still requires exact call/batch approval.
- Standing internal sub-agent authorization: spawn bounded internal sub-agents for reviews, validation, and drafting passes without per-spawn approval (ADR 0046). This never covers provider calls, Gates, approvals, or waivers — those remain human-owned.
